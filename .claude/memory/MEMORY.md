...
- [Tool override](tool-override.md) — hold Cmd/Ctrl for a temporary selection tool (restores tool + selection); tool lock forced permanently on and its three UI surfaces removed; zero fork; code shipped 2026-08-07 but BLOCKED on a found regression — forcing the lock disables vendor's auto-select-on-draw, breaking 29 e2e tests across 9 unrelated spec files; unresolved, needs a follow-up task
